...
#
package MS::BotBuilder::ApiClient;
...
sub _cfg {'MS::BotBuilder::Configuration'}
...
sub call_api {
    ...
    $self->{ua}->timeout($self->{http_timeout} || $MS::BotBuilder::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $MS::BotBuilder::Configuration::http_user_agent);
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("MS::BotBuilder::Object::$class")->new;
        if (ref $data eq "HASH") {
...
sub get_api_key_with_prefix
{
	...
	my $api_key = $MS::BotBuilder::Configuration::api_key->{$key_name};
	
	return unless $api_key;
	
	my $prefix = $MS::BotBuilder::Configuration::api_key_prefix->{$key_name};
